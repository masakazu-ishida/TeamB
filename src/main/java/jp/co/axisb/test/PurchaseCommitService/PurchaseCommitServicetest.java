package jp.co.axisb.test.PurchaseCommitService;
...
public class PurchaseCommitServicetest {
	@BeforeEach
	void init() {
	...
	public void testcartSum() {
		...
		try (Connection conn = ConnectionUtil.getConnectionForJUnit()) {
			...
			//idがuserのカート内を取得する
			int sum = PurchaseCommitService.cartSum("user");
	...
	//カート情報の取得、在庫の更新、注文と明細への登録、カートの削除
	@Test
	public void testcommitCartPurchase() {
		init();
		try (Connection conn = ConnectionUtil.getConnectionForJUnit()) {

			//idがuserのカート内を取得する
			int count = PurchaseCommitService.commitCartPurchase("user", "テスト");
